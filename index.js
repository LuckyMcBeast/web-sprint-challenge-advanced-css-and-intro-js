...
function get20s(array) {
  ...
  for (let key in array) {
    let dateSplit = (array[key].years).split(' ');
    if (Number(dateSplit[0]) >= 1900 && Number(dateSplit[2]) <= 2000) {
      in20s.push(array[key].name)
    }
  }
  return in20s;
}
...
function addArtist(array, artist) {
  artist.id = array.length + 1
  array.push(artist);
}

let me = {
  id: null,
  name: "Matthew Cullen McClellan",
  years: "1992 - Current Day",
  genre: "Full Stack Web Developer",
  nationality: "American",
  bio: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque semper, ligula ac faucibus rhoncus, nisl mauris aliquet dolor, quis porta ipsum tortor eu neque. Etiam nulla sem, fringilla quis velit vitae, consequat fringilla leo. Vestibulum maximus, leo id consequat semper, magna nibh convallis tellus, quis ultrices eros nunc et lacus."
}
...
function lotsOfArt(artists) {
  const lOArt = artists.filter(allTheArt => allTheArt.paintings > 100);
  return lOArt;
